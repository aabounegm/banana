...
allVarsDeclared prog = all
  ...
  (concatMap expressions (statements prog))
  where
    ...
    vars (FuncCallExpr (FuncCall _ arg)) = concatMap vars arg

    expressions (VarAssignment (VarAssign lhs rhs))  = vars lhs ++ vars rhs
    expressions (FuncCallStatement (FuncCall _ arg)) = concatMap vars arg
    expressions _                                    = []
